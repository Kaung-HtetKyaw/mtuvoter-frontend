    ...
    <v-card class="px-0 py-0 px-md-3 py-md-3 px-lg-5 py-lg-5">
      ...
      >
        <span
          ...
          >Are you sure to delete this FAQ ?</span
        >

        <p
          class="text-body-2 text-center my-0 font-weight-medium text--secondary mt-6 px-6"
        >
          Notice that the informations cannot be restored after the deletion.
        </p>
      </v-card-title>
  ...
  methods: {
    async deleteFAQ() {
      ...
      await store
        ...
        .then(() => {
          vm.loading = false;
          showNoti("success", "FAQ has been successfully deleted.");
        })
        .catch(() => {
          vm.loading = false;
