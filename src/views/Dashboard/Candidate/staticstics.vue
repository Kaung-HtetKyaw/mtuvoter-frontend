...
export default {
  ...
  methods: {
    async getCandidateVotePercentage() {
      ...
      vm.loading = true;
      return await axios()
        .get(`/ballots/elections/${election}/positions/${position}`)
        .then(res => {
          vm.loading = false;
    ...
    convertRawdataToPiechartData(raw) {
      ...
      let total_vote = raw.reduce(
        (accumulator, cur) => accumulator + cur.vote_count,
        0
      );
      return {
        series: [
          candidate_vote_by_position,
          total_vote - candidate_vote_by_position
        ],
