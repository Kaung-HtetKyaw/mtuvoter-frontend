    ...
    <v-row>
      ...
      >
        <v-container>
          ...
          >
            <div class="deep-purple--text darken-4 font-weight-medium text-h6">
          ...
          <v-row
            ...
            class="d-flex justify-center align-center flex-column"
          >
            <v-icon x-large color="deep-purple darken-4"
              >mdi-account-check-outline</v-icon
            >
            <p class="deep-purple--text darken-4 font-weight-bold text-h6">
              Your account is verified now.
            </p>
            <router-link
              :to="{ name: 'Login' }"
              class="text-decoration-underline text-body-1"
              >Log in to your account here</router-link
            >
          </v-row>
          <v-row
            v-if="error"
...
export default {
  ...
  async created() {
    const vm = this;
    await axios()
      .patch(`/users/verify/${this.$route.params.token}`)
      .then(() => {
        vm.loading = false;
        vm.verified = true;
      })
